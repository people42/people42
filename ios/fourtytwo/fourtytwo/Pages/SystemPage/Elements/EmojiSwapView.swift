...
struct EmojiSwapView: View {
        ...
        VStack {
            ...
            .onAppear {
                DispatchQueue.main.async {
                    if let index = emojis.firstIndex(of: userstate.emoji!) {
                        currentIndex = index
                    } else {
    ...
    private func changeEmoji(_ emoji: String ) {
        let data = ["emoji": emoji]
        AccountService.changeEmoji(data: data) { result in
            switch result {
            case .success( _):
                print("Emoji Changed!")
                userstate.emoji = emoji
                presentationMode.wrappedValue.dismiss()
...
struct SecondSectionView_Previews3: PreviewProvider {
